...
class Unregistered(User):
    ...
    def __init__(self, myID: str = "", bind_bot = None):
        super().__init__(myID, bind_bot)
        self.current_registration = None  # совместимость со старой логикой
        self.reg_process = None          # новый процесс регистрации (Process)
        self.in_registration = False

    class _BaseRegistration(core.Process):
        """Базовый класс для регистрационных процессов."""
    ...
    class TeacherRegistration(_BaseRegistration):
        """Пошаговая регистрация учителя: имя → фамилия → пароль → сохранение."""
        def __init__(self, ID):
            super().__init__(ID)
            self._chain = [
    ...
    class StudentRegistration(_BaseRegistration):
        """Пошаговая регистрация ученика: имя → фамилия → пароль → город → школа → класс → сохранение."""
        def __init__(self, ID):
            super().__init__(ID)
            self._chain = [
                self.ask_name, self.verify_name,
    ...
    def teacher_registration(self):
        ...
        # Новый процесс на базе core.Process
        ...
        # немедленно вывести первый вопрос
    ...
    def cancel_current_action(self):
        """Очищает состояние активного действия (регистрация и т.п.)."""
        try:
            if self.reg_process:
    ...
    def _cancelable_registration_execute(self):
        """Выполняет один шаг регистрационного процесса с поддержкой отмены."""
        if not self.reg_process:
            return False
        # передаём последний ввод в процесс
        self.reg_process.update_last_request(self._current_request)
        self.reg_process.execute()
        # если процесс завершён, очищаем состояние
        if getattr(self.reg_process, "completed", False):
            self.reg_process = None
            self.in_registration = False
            self._current_command = None
        return True
